...
Student::Student(int id, const std::string &name)
    : Student(id, name, {0.0f, 0.0f, 0.0f}) {}
...
Student::Student(const Student &other)
    : id(other.id), name(other.name),
      marks(std::make_unique<float[]>(SUBJECT_COUNT)), total(other.total),
      average(other.average), status(other.status), grade(other.grade) {
  if (other.marks) {
    for (int i = 0; i < SUBJECT_COUNT; ++i) {
...
Student::Student(Student &&other) noexcept
    : id(other.id), name(std::move(other.name)), marks(std::move(other.marks)),
      total(other.total), average(other.average), status(other.status),
      grade(other.grade) {
  other.id = 0;
  other.total = 0.0f;
  other.average = 0.0f;
...
void Student::display() const {
  std::cout << std::left << std::setw(10) << id << std::setw(20) << name
            ...
            << (marks ? marks[0] : 0.0f) << std::setw(8)
            << (marks ? marks[1] : 0.0f) << std::setw(8)
            << (marks ? marks[2] : 0.0f) << std::setw(8) << total
            << std::setw(8) << average << std::setw(8) << gradeToString(grade)
...
bool isBlank(const std::string &text) { return trim(text).empty(); }
...
int getValidatedInt(const std::string &prompt, int min, int max) {
  ...
  while (true) {
    ...
    if (input.empty()) {
      std::cout << "Invalid input. Please enter an integer between " << min
                << " and " << max << ".\n";
      continue;
    }
  ...
  while (true) {
    ...
    if (!input.empty()) {
      try {
        ...
        if (processedChars == input.length() && value >= 0.0f &&
            value <= 100.0f) {
          return value;
        }
      } catch (...) {
